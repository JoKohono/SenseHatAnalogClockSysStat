...
min_LED_current = sec_true*sec_stripe_length/60
...
def update_system():
    ...
    if cpufreq.max == cpufreq.current:
        CPU_f_color = green
    else:
        CPU_f_color = red
    psutil.cpu_percent()
    if psutil.cpu_percent() < 10:
        ...
        CPU_load_color = red
    return(CPU_load_color, CPU_f_color)      
    

#---------------------------------------------------------------------------------
#------------- and execute... ----------------------------------------------------
s.set_pixels(watchface_orbit)
while True: 
...
#            s.set_pixel((min_stripeX[min_LED_current-2]), min_stripeY[min_LED_current-2], old_pixel2)       
 ...
 #               s.set_pixel((sec_stripeX[sec_LED_current-1]), sec_stripeY[sec_LED_current-1], old_pixel1)
